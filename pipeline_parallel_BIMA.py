...
def run_inversion_loc(name:str, location:tuple, mdep:int, attempts:int):
    ...
    model_params = define_models.ModelParams(depth_limits=np.array([0., mdep]),
        ...
        method='disba',min_layer_thickness=1, head_wave=False, breadth=1,
            roughness = (0,0,0,0,0), #(1,1,4,4,0),
            to_0_grad = (0,0,0,0,0), to_m0 = (1000,1000,1000,1000,1000),
            q_model = './data/earth_models/qmod_highQ')
    ...
    sm = define_models.custom_starting_model(model_params, np.array([ 1, 3.65, 4.3, 4.5 ]), np.array([ 0, 4, 12, 21 ]), np.array(model_params.boundaries[1]))
...
def main(mdep:int=100, attempts:int=5):
    ...
    # lat = np.arange(23,25,0.1)
    # lon = np.arange(89.5,92.5,0.1)
    #
    ...
    model = run_inversion_loc(name, (24.,91.), mdep, attempts)
    ...
    plt.text(model.vsv[model.boundary_inds[2]+1] + 0.1,interface_z[2] + 15,'B3 depth=' + str(round(interface_z[2])) + ' km')
